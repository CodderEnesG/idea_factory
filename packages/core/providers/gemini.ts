 ...
 * "doğrulanmalı" der (felsefe zaten bu). Model config: GEMINI_MODEL (varsayılan gemini-3-flash;
 * yoksa gemini-2.5-flash'a düşür).
 */
export class GeminiProvider implements AnalystProvider {
  ...
  constructor(opts?: { apiKey?: string; model?: string }) {
    ...
    this.model = opts?.model ?? process.env["GEMINI_MODEL"] ?? "gemini-3-flash";
  }
