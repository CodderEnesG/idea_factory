...
describe("buildSignalBrief — niş/netlik ipucu", () => {
  ...
  it("narrow veya vague iken fit ≤ 79 uyarısı düşer, medium'da düşmez", () => {
    expect(buildSignalBrief(signal, stored)).not.toContain("niş/netlik kapısı");
  ...
  it("developer segmentinde geliştirici aracı tavanı uyarısı düşer", () => {
    expect(buildSignalBrief(signal, { ...stored, target_segment: "developer" })).toContain(
      "geliştirici aracı tavanı",
    );
  });
